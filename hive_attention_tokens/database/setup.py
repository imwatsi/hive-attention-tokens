...
from hive_attention_tokens.database.schema import DbSchema, DB_VERSION
...
class DbSession:
    ...
    def get_schema(self):
        for table in self._schema.tables:
            self.cur.execute(f"SELECT * FROM {table[0]} LIMIT 0")
            colnames = [desc[0] for desc in self.cur.description]
            self.live_schema[table[0]] = colnames
...
class DbSetup:
    ...
    def check_db(cls, config):
        ...
        try:
            ...
            cls.conn = psycopg2.connect(f"dbname=hive_attention_tokens user={config['db_username']} password={config['db_password']}")
        except psycopg2.OperationalError as e:
            if "hive_attention_tokens" in e.args[0] and "does not exist" in e.args[0]:
                print("No database found. Please create a 'hive_attention_tokens' database in PostgreSQL.")
                os._exit(1)
            else:
                print(e)
                os._exit(1)
    ...
    def _build_db(cls, schema):
        ...
        for table in schema.tables:
            print(f'Creating table: {table[0]}... ', end='')
            sql_stat = table[1]
