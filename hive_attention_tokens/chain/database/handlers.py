from hive_attention_tokens.chain.database.setup import DbSession
...
class AttentionTokensDb:
    ...
    def get_db_head(self):
        res = self._select('blocks', ['index', 'hash', 'timestamp'], order_by="index DESC", limit=1)
        if res:
            return res[0]
        else:
            return None
    ...
    def get_block(self, block_num):
        res = self._select('blocks',col_filters={'index': block_num})
        return self._populate_by_schema(res[0], self.schema['blocks'])
    
    # TRANSACTIONS

    def get_transactions_block(self, block_num):
        res = self._select('transactions',col_filters={'block_num': block_num})
        if res:
            return self._populate_by_schema_list(res, self.schema['transactions'])
        else:
            return []

    # HIVE ACCOUNTS

    def new_hive_account(self, acc_name, owner, active, posting, memo):
